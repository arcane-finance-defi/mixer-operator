...
pub fn event_loop(config: Config, mut receiver: mpsc::Receiver<MixClientRequest>, runtime: Runtime) {

    let mut client = runtime.block_on(MixerClient::new(
    ...
    loop {
        ...
        match request {
            MixClientRequest::Mix { note_file, account_id, response_sink } => {
                let result = runtime.block_on(
                    client.mix(note_file, account_id)
                );
